...
class XGC:
    ...
    def f0_diag_vol(self, f0_inode1, ndata, isp, f0_f, progress=False):
        """ 
        Input:
        f0_inode1: int
        ...
        return (vol_, vth[:,np.newaxis,np.newaxis], vp[np.newaxis,np.newaxis,:], mu[np.newaxis,:,np.newaxis], vth2[:,np.newaxis,np.newaxis], ptl_mass[isp], sml_e_charge, f0_grid_vol, mu_vp_vol)
    
    def f0_diag_vol_origdim(self, f0_inode1, ndata, isp, f0_f, progress=False):
        """ 
        Input:
        f0_inode1: int
        ndata: int (f0_inode2=f0_inode1+ndata)
        ...
        return (vol_, vth, vp, mu, vth2, ptl_mass[isp], sml_e_charge, f0_grid_vol, mu_vp_vol)
...
#        print(np.sum(f0_f.astype(np.float128)*mu[np.newaxis,:,np.newaxis], axis=(1,2))[0] , vth2[0], ptl_mass[isp])
#        print(mu)
